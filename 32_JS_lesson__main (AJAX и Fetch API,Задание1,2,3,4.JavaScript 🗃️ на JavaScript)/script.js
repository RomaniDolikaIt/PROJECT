...
document
  ...
  .addEventListener("submit", async (event) => {
    ...
    try {
      ...
      document.getElementById("post-response").textContent =
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (cardToDelete) {
        cardToDelete.remove();
        alert(
          `Пост №${postId} успешно удален! (Статус сервера: ${response.status})`,
        );
      }
    }
  } catch (error) {
...
document
  ...
  .addEventListener("submit", async (event) => {
    ...
    try {
      ...
      document.getElementById("user-response").textContent =
        "Ошибка PUT запроса: " + error.message;
    }
  });
